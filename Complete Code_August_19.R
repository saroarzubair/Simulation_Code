## STEP 1: ASE pre-data estimation (quarterly collision counts)
#Install packages
library(sf)
library(dplyr)
library(lubridate)
...
# Import Toronto police-reported collision data (All_Collisions_Open_Data_(ASR-T-TBL-001)
all_collision <- read_excel("All Collision_2014-2023.xlsx")

# Import ASE location data from Toronto Transportation Services' ASE program phase 1 to phase 5 
aSE_locations <- read_excel("ASE Camera_Location_P1-6.xlsx")

# Convert to sf objects with WGS 84 CRS
ase_sf <- st_as_sf(ASE_locations, coords = c("long", "lat"), crs = 4326)
collision_sf <- st_as_sf(all_collision, coords = c("LONG_WGS84", "LAT_WGS84"), crs = 4326)

# Reproject to meters for buffering 
ase_sf <- st_transform(ase_sf, 32617)
collision_sf <- st_transform(collision_sf, 32617)

#Buffer distance downstream (change according to distance)
ase_sf$bearing <- 90
# Convert coordinates to matrix (long-lat, WGS84) for geosphere::destPoint
ase_latlon <- st_transform(ase_sf, 4326)
coords <- st_coordinates(ase_latlon)

# Generate downstream endpoint using geosphere
downstream_coords <- geosphere::destPoint(p = coords, b = ase_sf$bearing, d = 1000)

# Combine original and downstream point to make a directional buffer line
ase_sf$X1 <- coords[, 1]
ase_sf$Y1 <- coords[, 2]
ase_sf$X2 <- downstream_coords[, 1]
ase_sf$Y2 <- downstream_coords[, 2]
# Function to build a narrow rectangle downstream
make_rect <- function(x1, y1, x2, y2, width = 100) {
  dx <- x2 - x1
  dy <- y2 - y1
  len <- sqrt(dx^2 + dy^2)
  ux <- dx / len
  uy <- dy / len
  wx <- -uy * width / 2 / 111320  # Approximate meter-to-degree conversion
  wy <- ux * width / 2 / 111320
  matrix(c(
    x1 + wx, y1 + wy,
    x1 - wx, y1 - wy,
    x2 - wx, y2 - wy,
...
# Apply to all ASE cameras
buffer_polys <- lapply(1:nrow(ase_sf), function(i) {
  coords <- make_rect(ase_sf$X1[i], ase_sf$Y1[i], ase_sf$X2[i], ase_sf$Y2[i], width = 100)
  st_polygon(list(coords))
})

# Convert to sf
directional_buffers <- st_sfc(buffer_polys, crs = 4326) %>% st_transform(32617)

# Join files
## collisions_in_downstream <- st_join(collision_sf, st_sf(ase_sf, geometry = directional_buffers), join = st_within)

collisions_near_ase <- st_join(collision_sf, buffer_50m, join = st_within)

# Quaterly collision counts
collisions_near_ase <- collisions_near_ase %>%
  mutate(OCC_DATE = as.Date(OCC_DATE),
         quarter = paste0(year(OCC_DATE), "-Q", quarter(OCC_DATE))) %>%
  filter(OCC_DATE >= as.Date("2015-01-01"))

quarterly_counts <- collisions_near_ase %>%
  st_drop_geometry() %>%
  group_by(`Location Code`, Phase, quarter) %>%
  summarise(collision_count = n(), .groups = "drop")

# Create full grid of Location × Quarter from 2015 Q1 to 2023 Q4
all_quarters <- seq.Date(from = as.Date("2015-01-01"), to = as.Date("2023-12-31"), by = "quarter")
quarter_labels <- paste0(year(all_quarters), "-Q", quarter(all_quarters))

ase_phase_lookup <- ASE_locations %>%
  dplyr::select(`Location Code`, `Phase`) %>%
  distinct()

location_quarter_grid <- expand_grid(
  `Location Code` = unique(ASE_locations$`Location Code`),
  quarter = unique(quarter_labels)
) %>%
  left_join(ase_phase_lookup, by = "Location Code")

quarterly_counts_complete <- location_quarter_grid %>%
  left_join(quarterly_counts, by = c("Location Code", "Phase", "quarter")) %>%
  mutate(collision_count = replace_na(collision_count, 0))

# Transform data for power calculation by adding appropriate dates and pandemic variable
pre_data <- df %>%
  rename(LocationCode = `Location Code`,
         Phase_ASE = Phase,
  ...
  mutate(
    ...
    Date = case_when(
      Qtr == 1 ~ ymd(paste0(Year, "-01-01")),
      Qtr == 2 ~ ymd(paste0(Year, "-04-01")),
      Qtr == 3 ~ ymd(paste0(Year, "-07-01")),
      Qtr == 4 ~ ymd(paste0(Year, "-10-01"))
    ),
    Q1 = ifelse(Qtr == 1, 1, 0),
    Q2 = ifelse(Qtr == 2, 1, 0),
    Q3 = ifelse(Qtr == 3, 1, 0),
    Q4 = ifelse(Qtr == 4, 1, 0),
    Date_CenterQuarter = Date + months(1) + days(6),
    Pandemic = ifelse(Year >= 2020, 1, 0),
    INVTYPE = "Pre"
  ) %>%
  group_by(LocationCode) %>%
  arrange(Date, .by_group = TRUE) %>%
  mutate(T = row_number() - max(row_number())) %>%
  ungroup() %>%
  select(LocationCode, Phase_ASE, YQ, Collisions, Year, Date,
...
data$Date_CenterQuarter <- as.Date(data$Date_CenterQuarter)
data$Collisions <- as.numeric(data$Collisions)

# ----- helper ticks & padding -----
y_step   <- 1000
y_max    <- max(c(data$q0.975, data$Collisions), na.rm = TRUE)
y_breaks <- seq(0, ceiling(y_max / y_step) * y_step, by = y_step)

# Quarters you already use as breaks
x_breaks <- as.Date(c("2015-07-01","2015-10-01","2016-01-01","2016-04-01",
...
x_labels <- c("4","3","2","1","4","3","2","1","4","3","2","1",
              "4","3","2","1","4","3","2","1","1","2","3","4")

grp <- tibble::tibble(
  start = as.Date(c("2015-07-01","2016-07-01","2017-07-01",
                    "2018-07-01","2019-07-01","2020-07-01")),
  end   = as.Date(c("2016-07-01","2017-07-01","2018-07-01",
                    "2019-07-01","2020-07-01","2021-04-01")),
  lab   = c("Pre-Y5Quarter","Pre-Y4Quarter","Pre-Y3Quarter",
            "Pre-Y2Quarter","Pre-Y1Quarter","**ASE-Y1Quarter")
) %>%
  mutate(x = start + (end - start)/2)

# vertical placement for the group labels (below the axis)
y_bottom <- min(y_breaks) - 0.08 * diff(range(y_breaks))

p <- ggplot(data, aes(x = Date_CenterQuarter)) +
  geom_ribbon(aes(ymin = q0.025, ymax = q0.975,
                  fill = "Expected count\n95% credible interval"),
              alpha = 0.75, show.legend = TRUE) +
  geom_line(aes(y = (q0.025 + q0.975)/2,
                colour = "Expected count",
                linetype = "Expected count"),
            size = 1.1, show.legend = TRUE) +
  geom_line(aes(y = Collisions,
                colour = "Observed count",
                linetype = "Observed count"),
            alpha = 0.75, size = 1.2, show.legend = TRUE) +
  geom_vline(xintercept = ase_date, linetype = 2, size = 1, colour = "red") +
  annotate("text", x = ase_date,
           y = max(y_breaks) + 0.05 * max(y_breaks),
           label = "ASE Implementation", fontface = "bold", size = 5, vjust = 0) +
  scale_x_date(breaks = x_breaks, labels = x_labels, expand = expansion(mult = c(0, 0.02))) +
  scale_y_continuous(breaks = y_breaks, expand = expansion(mult = c(0, 0.10))) +
  scale_fill_manual(NULL, values = c("Expected count\n95% credible interval" = "steelblue2")) +
  scale_colour_manual(NULL, values = c("Observed count" = "black", "Expected count" = "blue")) +
  scale_linetype_manual(NULL, values = c("Observed count" = "solid", "Expected count" = "dashed")) +
  guides(
    fill = guide_legend(order = 3, override.aes = list(alpha = 0.75)),
    colour = guide_legend(order = 1),
    linetype = "none"
  ) +
  labs(x = NULL, y = "Number of Collisions",
       caption = paste0(
         "* 475 camera units each pre-quarter\n",
         "** ASEQ1 – 475 cameras, Q2 – 401 cameras, Q3 – 201 cameras and Q4 – 118 cameras"
       )) +
  theme(
    ...
    plot.margin        = margin(t = 18, r = 8, b = 40, l = 8)
  ) +
  coord_cartesian(clip = "off") +
  geom_text(data = grp, aes(x = x, y = y_bottom, label = lab),
            inherit.aes = FALSE, size = 4)

ggsave(
  filename = "figure_600dpi.png",
  plot = last_plot(),
  scale = 1,
  width = 6000, 
  height = 3000,
